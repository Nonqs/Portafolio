  ...
  return (
    <section ref={projectRef}>
      {projects.map((project, index) => (
        <div
          key={index}
          className={`flex flex-col overflow-hidden xl:flex-row justify-center mb-20 h-[80h] rounded-xl drop-shadow-2xl backdrop-blur-sm py-2 lg:py-0 ${
            isVisible ? "animate-fade-in" : "opacity-0"
          }`}
          style={{ borderTop: "#01c38d 8px solid" }}
        >
          <Background />
          {project.github && (
            <a
            ...
            >
              <div
                className={`p-1 bg-white border border-black drop-shadow-lg me-2 mb-2 rounded-xl hover:drop-shadow-2xl transform hover:scale-110 transition-transform duration-200 ease-in-out`}
              >
          ...
          {project.web && (
            <a
              className="absolute top-1 left-16 z-50"
              href={project.web}
              target="_blank"
              rel="noopener noreferrer"
            >
          ...
          <div className="w-3/7 h-full flex flex-col  mt-0 lg:mt-10 ms-10 me-10">
            <article className="flex flex-col mb-5">
              <h4
                className={`text-[#01c38d] mb-2 text-xl md:text-4xl font-semibold p-2 border-animated-right text-center md:text-left ${
      ...
      <article className="w-full flex flex-col justify-center items-center mb-20">
        ...
        >
          <div
            className={`p-1 bg-white border border-black flex justify-center items-center w-28 drop-shadow-lg me-2 mb-2 rounded-xl hover:drop-shadow-2xl transform hover:scale-110 transition-transform duration-200 ease-in-out`}
          >
            <img className="w-10" src={github} alt="" />
            <span>GitHub</span>
          </div>
